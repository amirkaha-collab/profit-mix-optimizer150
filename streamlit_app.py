...
<div class='tb'>
  ...
  <div class='tb-right'>
    ...
    nb1, nb2, nb3, nb_sp = st.columns([1, 1.4, 1.2, 6])
    with nb1:
        if st.button("🏠 בית", key=f"nav_home_{active}", use_container_width=True,
            ...
            st.rerun()
    with nb2:
        if st.button("📊 ניתוח תיק", key=f"nav_pf_{active}", use_container_width=True,
                     type="primary" if active == "portfolio" else "secondary"):
...
def render_landing_page(df_long: "pd.DataFrame"):
    """
    Premium landing page — app_page == 'home'.
    Ends with st.stop(). Zero changes to any existing logic.
    """
    st.markdown(_LANDING_CSS, unsafe_allow_html=True)
...
<div class="hero r">
  <div class="hero-inner">
    <div class="hero-badge"><span class="hero-bdot"></span>Investment Intelligence Platform</div>
    <div class="hero-co">Profit Financial Group</div>
    <h1 class="hero-h1">ניהול השקעות<br><span class="ac">בדיוק מקצועי</span></h1>
    <div class="hero-prod">Alpha Optimizer by Profit</div>
    <p class="hero-sub">
      פלטפורמה מתקדמת לאופטימיזציית תמהיל מסלולי השקעה,
      ניתוח מעמיק של תיקי לקוחות ועיון בנתוני אסטרטגיות מוסדיות —
      על פני כלל עולמות המוצרים הפנסיוניים.
    </p>
    <div class="hero-pills">
      <span class="hero-pill">🏦 קרנות השתלמות</span>
      <span class="hero-pill">📋 פוליסות חיסכון</span>
      <span class="hero-pill">🏛️ קרנות פנסיה</span>
      <span class="hero-pill">💼 קופות גמל</span>
      <span class="hero-pill">⚙️ אופטימיזציה</span>
      <span class="hero-pill">📐 ניתוח מוסדי</span>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
    ...
    st.markdown("""
<div class="sh">
  <div class="sh-eye">כניסה לפי עולם מוצר</div>
  <div class="sh-h2">בחר עולם מוצר להתחיל</div>
  <p class="sh-desc" style="margin-bottom:30px">
    כל עולם מוצר כולל: אופטימיזציית תמהיל מסלולים · ניתוח אסטרטגיות מוסדיות היסטוריות · השוואת קרנות ומנהלים
  </p>
</div>""", unsafe_allow_html=True)

    pw1, pw2, pw3, pw4 = st.columns(4, gap="medium")
    _worlds = [
        (pw1, "🏦", "קרנות השתלמות", "קרנות השתלמות", "#3A7AFE", "#EFF6FF", "#1F3A5F",
         "אופטימיזציה מתקדמת של תמהיל מסלולי קרנות השתלמות עם ניתוח מוסדי", ["מניות", 'חו"ל', "שארפ", "שירות"]),
        (pw2, "📋", "פוליסות חיסכון", "פוליסות חיסכון", "#10B981", "#ECFDF5", "#065F46",
         "בניית תמהיל מיטבי לפוליסות חיסכון ואנליזה מוסדית מעמיקה", ["פוליסה", "מנהל", "מסלול", "חשיפה"]),
        (pw3, "🏛️", "קרנות פנסיה", "קרנות פנסיה", "#F59E0B", "#FFFBEB", "#92400E",
         "אופטימיזציה ובחינת אסטרטגיות מוסדיות ב-קרנות הפנסיה", ["פנסיה", "ביטוח", "מסלול", "שארפ"]),
        (pw4, "💼", "קופות גמל", "קופות גמל", "#8B5CF6", "#F5F3FF", "#5B21B6",
         "ניתוח תמהיל, ביצועים וחשיפות בקופות הגמל לסוגיהן", ["גמל", "מנהל", "חשיפה", 'חו"ל']),
    ]
    for col, icon, title, pt, accent, light, dark, desc, caps in _worlds:
        with col:
            caps_html = "".join(f"<span class='pw-tag' style='--lc:{light};--dc:{dark}'>{c}</span>" for c in caps)
            st.markdown(f"""
<div class="pw r" style="--ac:{accent};--lc:{light};--dc:{dark}">
  <div class="pw-bar"></div>
  <div class="pw-shine"></div>
  <div class="pw-ico" style="--lc:{light}">{icon}</div>
  <div class="pw-title">{title}</div>
  <div class="pw-desc">{desc}</div>
  <div class="pw-tags">{caps_html}</div>
</div>""", unsafe_allow_html=True)
            if st.button(f"כנס ←", key=f"lp_pw_{pt}", use_container_width=True, type="primary"):
                _nav_to("app", pt)
                st.rerun()
...
<div class="sh" style="padding-top:0">
  <div class="sh-eye">זרימה נפרדת</div>
  <div class="sh-h2">ניתוח תיק לקוח</div>
  <p class="sh-desc" style="margin-bottom:30px">
    ניתוח רוחבי עצמאי על פני כלל עולמות המוצרים — ייבוא מסלקה, איגוד חשיפות, זיהוי פערים, תכנון שינויים, הפקת דוחות
  </p>
</div>""", unsafe_allow_html=True)

    pf_col, pf_info = st.columns([1.3, 2.7], gap="large")
    with pf_col:
        st.markdown("""
<div class="pf-hero">
  <div class="pf-badge">✦ ניתוח מקיף</div>
  <span class="pf-icon">📊</span>
  <div class="pf-title">ניתוח תיק לקוח</div>
  <div class="pf-desc">
    ייבא דוח מסלקה פנסיונית, קבל ניתוח מעמיק של כלל האחזקות,
    תכנן שינויים ברמת מוצר ומסלול, והפק דוח מקצועי ללקוח.
  </div>
  <div class="pf-caps">
    <span class="pf-cap">ייבוא מסלקה</span>
    <span class="pf-cap">חשיפות</span>
    <span class="pf-cap">תכנון</span>
    <span class="pf-cap">דוחות</span>
    <span class="pf-cap">NotebookLM</span>
  </div>
</div>""", unsafe_allow_html=True)
        if st.button("פתח ניתוח תיק ←", key="lp_btn_portfolio", use_container_width=True, type="primary"):
            _nav_to("app", "תיק לקוח")
            st.rerun()

    with pf_info:
        feats = [
            ("ייבוא אוטומטי", "מדוח המסלקה הפנסיונית (XLSX) — זיהוי מוצרים וסיווג אוטומטי"),
            ("איגוד חשיפות", 'חישוב תמהיל משוקלל מקיף: מניות, חו"ל, מט"ח, לא-סחיר על פני כלל המוצרים'),
            ("ניתוח גרפי", "גרפים אינטראקטיביים: פיזור מנהלים, תמהיל נכסים, חשיפות"),
            ("זיהוי פערים", "השוואה בין המצב הקיים לבין יעדי חשיפה — זיהוי עודפים וחסרים"),
            ("תכנון שינויים", "מסך תכנון ייעודי — בחירת חלופות, השוואת לפני/אחרי, תכנון ניוד"),
            ("הפקת דוחות", "דוח HTML מעוצב · Jupyter Notebook · NotebookLM Package · CSV"),
        ]
        feats_html = "".join(
            f"<div class='pf-row'><div class='pf-dot'></div>"
            f"<div class='pf-row-text'><b>{t}</b> {d}</div></div>"
            for t, d in feats
        )
        st.markdown(f"""
<div class="pf-panel">
  <div class="pf-panel-lbl">מה כולל הניתוח</div>
  {feats_html}
</div>""", unsafe_allow_html=True)
    ...
    st.markdown("""
<div class="sh" style="padding-top:0">
  <div class="sh-eye">תהליכי עבודה</div>
  <div class="sh-h2">שלושת מסלולי העבודה</div>
  <p class="sh-desc" style="margin-bottom:30px">
    כל מסלול עצמאי ומותאם לשאלה שונה — בחר את המסלול המתאים לצורך הנוכחי
  </p>
</div>""", unsafe_allow_html=True)

    wf1, wf2, wf3 = st.columns(3, gap="medium")
    _wfs = [
        (wf1, "⚙️", "EFF6FF", "אופטימיזציית תמהיל", "מה המסלול האופטימלי עבורי?", [
            "בחר עולם מוצר: השתלמות, פנסיה, פוליסה, גמל",
            "הגדר יעדי חשיפה בסליידרים",
            "הפעל חישוב — המערכת בוחנת אלפי שילובים",
            "עיין ב-4 חלופות: דיוק · שארפ · שירות · משוקלל",
            "בחר, הצבע, הפק דוח ייעוץ",
        ]),
        (wf2, "📊", "ECFDF5", "ניתוח תיק לקוח", "מה מצב התיק הקיים?", [
            "העלה דוח מסלקה פנסיונית (XLSX)",
            "המערכת מסווגת ומשלימה נתוני חשיפה",
            "עיין בגרפים: תמהיל, מנהלים, חשיפות",
            "זהה פערים ותכנן שינויים",
            "עבור לאופטימיזציה אם נדרש",
            "הפק דוח מקצועי ללקוח",
        ]),
        (wf3, "📐", "FFF7ED", "ניתוח אסטרטגיות", "מה עושים המוסדיים?", [
            "בחר עולם מוצר → גלול לסקציית אסטרטגיות",
            "בחר גוף מוסדי ומסלול השקעה",
            'בחר רכיב אלוקציה: מניות / חו"ל / מט"ח',
            "עיין בגרף סדרת הזמן ההיסטורית",
            "השווה בין גופים וזהה מגמות ואנומליות",
        ]),
    ]
    for col, icon, bg, title, sub, steps in _wfs:
        with col:
            steps_html = "".join(
                f"<div class='wf-step'><span class='wf-num'>{i+1}</span><span class='wf-txt'>{s}</span></div>"
                for i, s in enumerate(steps)
            )
            st.markdown(f"""
<div class="wf">
  <div class="wf-hdr">
    <div class="wf-ico" style="--wb:#{bg}">{icon}</div>
    <div><div class="wf-name">{title}</div><div class="wf-quest">{sub}</div></div>
  </div>
  {steps_html}
</div>""", unsafe_allow_html=True)
    ...
    st.markdown("""
<div class="sh" style="padding-top:0">
  <div class="sh-eye">הסבר מפורט</div>
  <div class="sh-h2">מה עושה כל מודול</div>
  <p class="sh-desc" style="margin-bottom:30px">הבנת העומק של כל מודול — לפני שמתחילים</p>
</div>""", unsafe_allow_html=True)
    ...
    with ex1:
        st.markdown("""
<div class="exp exp-b">
  <div class="exp-hdr"><span class="exp-emo">📐</span><div class="exp-title">ניתוח אסטרטגיות מוסדיות</div></div>
  <div class="exp-body">
    המודול חושף את <strong>אסטרטגיית האלוקציה ההיסטורית</strong> של הגופים המוסדיים הגדולים —
    הראל, מגדל, כלל, מנורה ועוד — ברמה שנתית וחודשית.
    ניתן לזהות תבניות, אנומליות ומגמות שאינן גלויות בנתון נקודתי.
  </div>
  <ul class="exp-ul">
    <li>מעקב היסטורי אחר שינויי חשיפה לאורך מחזורי שוק</li>
    <li>השוואת אסטרטגיות בין מנהלים מובילים על אותו מסלול</li>
    <li>זיהוי תזמון כניסה ויציאה מחשיפות מניות / חו"ל</li>
    <li>ניתוח פערי אסטרטגיה ואנומליות בהחלטות ניהול</li>
    <li>בסיס דאטה מוסדי להחלטות ניוד ייעוציות מבוססות</li>
  </ul>
</div>""", unsafe_allow_html=True)
    ...
    with ex2:
        st.markdown("""
<div class="exp exp-g">
  <div class="exp-hdr"><span class="exp-emo">📊</span><div class="exp-title">ניתוח תיק לקוח</div></div>
  <div class="exp-body">
    המודול מספק <strong>תמונה מלאה ומאוחדת</strong> של תיק הלקוח —
    מייבוא אוטומטי מהמסלקה, דרך סיווג, השלמה ואיגוד נתונים,
    ועד לזיהוי פערים ותכנון שינויים אסטרטגיים.
  </div>
  <ul class="exp-ul">
    <li>ייבוא, סיווג ואיגוד נתונים על פני כלל עולמות המוצרים</li>
    <li>זיהוי חוסרים ועודפי חשיפה ביחס לפרופיל הסיכון הרצוי</li>
    <li>תכנון שינויים ברמת מוצר ומסלול עם השוואת לפני/אחרי</li>
    <li>השלמה אוטומטית של נתוני חשיפה חסרים מהמאגר</li>
    <li>הפקת דוחות מקצועיים מוכנים לשיתוף עם הלקוח</li>
  </ul>
</div>""", unsafe_allow_html=True)
    ...
    st.markdown("""
<div class="sh" style="padding-top:0">
  <div class="sh-eye">הכנה מוקדמת</div>
  <div class="sh-h2">מה כדאי להכין</div>
  <p class="sh-desc" style="margin-bottom:30px">ארבעה פריטים שיאפשרו לך למצות את המערכת במלואה</p>
</div>""", unsafe_allow_html=True)
    st.markdown("""
<div class="ck-grid r">
  <div class="ck"><span class="ck-ico">📥</span>
    <div class="ck-body"><div class="ck-title">דוח מסלקה (XLSX)</div>
    <div class="ck-desc">קובץ ייצוא ממסלקת המוצרים הפנסיוניים — נדרש לניתוח תיק קיים ולהשוואת לפני/אחרי</div></div>
  </div>
  <div class="ck"><span class="ck-ico">🎯</span>
    <div class="ck-body"><div class="ck-title">יעדי חשיפה</div>
    <div class="ck-desc">אחוזי מניות, חו"ל, מט"ח, לא-סחיר — המספרים שמגדירים את פרופיל הסיכון של הלקוח</div></div>
  </div>
  <div class="ck"><span class="ck-ico">🏢</span>
    <div class="ck-body"><div class="ck-title">העדפות מנהלים</div>
    <div class="ck-desc">האם מותר לערבב גופים? האם יש מגבלת מנהל? כמה קרנות לשלב? מה המדיניות?</div></div>
  </div>
  <div class="ck"><span class="ck-ico">💰</span>
    <div class="ck-body"><div class="ck-title">היקפי השקעה</div>
    <div class="ck-desc">סכום כולל לחלוקה — נדרש לחישוב משקל קרן נעולה ולדוח כספי ערכי</div></div>
  </div>
</div>
""", unsafe_allow_html=True)
    ...
    # ── GUIDE CTA ────────────────────────────────────────────────────
    g_col, _ = st.columns([2.2, 3.8])
    with g_col:
        st.markdown("""
<div class="gcta">
  <div class="gcta-title">📖 מדריך שימוש מלא</div>
  <div class="gcta-desc">הוראות שלב-אחר-שלב לכל תהליך עבודה — אופטימיזציה, ניתוח תיק, ניתוח מוסדי, הפקת דוחות</div>
</div>""", unsafe_allow_html=True)
        if st.button("פתח מדריך שימוש ←", key="lp_guide_btn", use_container_width=True):
            st.session_state["app_page"] = "guide"
            st.rerun()
    ...
    st.markdown("""
<div class="disc">
  <div class="disc-hdr">⚠️ הצהרה חשובה — גרסת בטא</div>
  <div class="disc-badges">
    <span class="disc-badge">BETA VERSION</span>
    <span class="disc-badge">לא ייעוץ פיננסי</span>
    <span class="disc-badge">ייתכנו שגיאות</span>
    <span class="disc-badge">נדרשת בדיקה מקצועית</span>
  </div>
  <ul class="disc-ul">
    <li><strong>גרסת בטא:</strong> המערכת בפיתוח פעיל. ייתכנו באגים, שגיאות לא צפויות ושינויים ללא התראה.</li>
    <li><strong>שגיאות סיווג:</strong> המערכת עלולה לסווג מוצרים בצורה שגויה — יש לאמת ידנית.</li>
    <li><strong>שגיאות חישוב:</strong> חישובי חשיפה וניתוחים מבוצעים על בסיס הנחות — לא תמיד מדויקים.</li>
    <li><strong>נתונים חסרים:</strong> חלק מהנתונים מבוססים על מידע ציבורי חלקי ועלולים להיות מיושנים.</li>
    <li><strong>פלטים חלקיים:</strong> חלק מהתוצאות מבוססות על השלמה אוטומטית ועלולות להיות לא שלמות.</li>
    <li><strong>לא ייעוץ פיננסי:</strong> אין בתוצאות המערכת המלצה אישית או תחליף לייעוץ השקעות מקצועי.</li>
    <li><strong>אחריות המשתמש:</strong> יש לאמת כל נתון מול מקורות רשמיים לפני כל החלטת השקעה.</li>
  </ul>
</div>
""", unsafe_allow_html=True)
